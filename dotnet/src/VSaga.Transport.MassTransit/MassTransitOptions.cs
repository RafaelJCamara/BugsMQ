...
public sealed class MassTransitOptions
{
    ...
    /// <summary>
    /// The single durable topic exchange every VSaga message published through this adapter travels
    /// over, mirroring <c>RabbitMqOptions.ExchangeName</c>. Every message shares one MassTransit message
    /// contract (<see cref="VSagaEnvelopeMessage"/>) forced onto this exchange name via
    /// <c>cfg.Message&lt;VSagaEnvelopeMessage&gt;(m =&gt; m.SetEntityName(...))</c> — VSaga's own
