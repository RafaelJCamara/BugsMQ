...
namespace BugsMQ.Transport.Brighter.Tests;
...
#pragma warning restore CA1001
    ...
    public async Task InitializeAsync()
    {
        ...
        var options = new BrighterOptions
        {
            ...
            ExchangeName = "bugsmq.saga.events.brighter.test",
        };
    ...
    public async Task PublishAndSubscribe_DeliversMessageWithCorrelationAndType()
    {
        ...
        var subscription = new TransportSubscription("TestConsumer", [typeof(PingMessage)], "bugsmq.brighter.test.ping-queue");
    ...
    public async Task Send_DeliversDirectlyToNamedQueueWithoutExchange()
    {
        ...
        var subscription = new TransportSubscription("TestConsumer2", [typeof(PingMessage)], "bugsmq.brighter.test.direct-queue");
        using var handle = await _transport.SubscribeAsync(subscription, async (received, ct) =>
        {
            tcs.TrySetResult(received);
            await received.Ack.AckAsync(ct);
        ...
        await _transport.SendAsync("bugsmq.brighter.test.direct-queue", new PingMessage("direct"), MessageEnvelope.New(correlationId));
    ...
    public async Task PublishAndSubscribe_PropagatesAllFourBugsMqHeadersUnchanged()
    {
        ...
        var subscription = new TransportSubscription("TestConsumer3", [typeof(PingMessage)], "bugsmq.brighter.test.headers-queue");
        using var handle = await _transport.SubscribeAsync(subscription, async (received, ct) =>
        {
            tcs.TrySetResult(received);
            await received.Ack.AckAsync(ct);
