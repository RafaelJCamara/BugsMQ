...
/// <summary>
/// Resolves <see cref="BugsMqDbContext"/> lazily from a fresh scope rather than taking it as a
/// constructor dependency: BugsMQ.Dashboard.Api.Tests' DashboardApiFactory removes it from DI entirely
...
public sealed class PostgresHealthCheck(IServiceScopeFactory scopeFactory) : IHealthCheck
{
    public async Task<HealthCheckResult> CheckHealthAsync(HealthCheckContext context, CancellationToken cancellationToken = default)
    {
        ...
        var db = scope.ServiceProvider.GetService<BugsMqDbContext>();
        if (db is null)
