...
public static class SagaEndpoints
{
    public static void MapSagaEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapPost("/{correlationId:guid}/retry", async (Guid correlationId, ISagaSummaryReader reader, ISagaEventLogStore log, ISagaAdminStore admin, IMessageTransport transport, CancellationToken ct) =>
        {
            var summary = await reader.GetAsync(correlationId, ct);
            if (summary is null)
                return Results.NotFound();
            ...
            if (resetToState != summary.CurrentState)
