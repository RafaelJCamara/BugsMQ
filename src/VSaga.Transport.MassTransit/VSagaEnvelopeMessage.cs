...
/// <summary>
/// The single MassTransit message contract every VSaga message travels as. VSaga message types are
...
/// <c>IConsumer&lt;T&gt;</c>) is built around. Rather than fight that, every VSaga publish/send carries
...
/// four VSaga envelope headers still ride on MassTransit's own <c>SendContext.Headers</c>/
...
/// </summary>
public sealed record VSagaEnvelopeMessage(string MessageTypeName, byte[] Body);
