...
/// per <see cref="SubscribeAsync"/> call. Every message — regardless of its real BugsMQ message type — is
...
/// BugsMQ.Core's SagaOrchestrator already owns retry, redelivery, compensation, and dedup (see
    ...
    ILogger<WolverineTransport> logger) : IMessageTransport
{
    ...
    private async Task PublishInternalAsync(string messageTypeName, byte[] body, MessageEnvelope envelope, Uri destination, CancellationToken cancellationToken)
    {
        ...
        // Everything BugsMQ needs travels inside this self-contained wire payload rather than relying on
