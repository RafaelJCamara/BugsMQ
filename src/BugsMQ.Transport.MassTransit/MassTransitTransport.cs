...
/// <see cref="IMessageTransport"/>'s own doc comment). Every BugsMQ message travels as one fixed
/// MassTransit contract, <see cref="BugsMqEnvelopeMessage"/> — see its doc comment for why — with the
/// four BugsMQ envelope headers (and correlation/message id) riding on MassTransit's own
    ...
    ILogger<MassTransitTransport> logger) : IMessageTransport
{
    ...
    {
        var envelopeMessage = new BugsMqEnvelopeMessage(messageTypeName, body);
    ...
    private static void ApplyEnvelope(SendContext<BugsMqEnvelopeMessage> ctx, MessageEnvelope envelope)
    {
    ...
    public async Task<IDisposable> SubscribeAsync(TransportSubscription subscription, Func<ReceivedMessage, CancellationToken, Task> handler, CancellationToken cancellationToken = default)
    {
        var handle = bus.ConnectReceiveEndpoint(subscription.QueueNameHint, e =>
        {
            ...
            // Explicit topology only: every BugsMQ message type shares the one MassTransit contract
            // (BugsMqEnvelopeMessage), so the default auto-bind-on-consume would deliver every BugsMQ
            // message ever published to every subscriber regardless of which types it declared.
            e.ConfigureConsumeTopology = false;
            e.Consumer(() => new BugsMqEnvelopeConsumer(handler, subscription.ConsumerName, logger));

            if (e is IRabbitMqReceiveEndpointConfigurator rabbitMqEndpoint)
            {
                foreach (var messageType in subscription.MessageTypes)
                {
                    rabbitMqEndpoint.Bind<BugsMqEnvelopeMessage>(x =>
                    {
    ...
    /// decision, and <see cref="BugsMqEnvelopeConsumer.Consume"/> turns a recorded nack (or no decision at
    ...
    private sealed class BugsMqEnvelopeConsumer(
        Func<ReceivedMessage, CancellationToken, Task> handler,
        string consumerName,
        ILogger logger) : IConsumer<BugsMqEnvelopeMessage>
    {
        public async Task Consume(ConsumeContext<BugsMqEnvelopeMessage> context)
        {
    ...
    /// <summary>Thrown by <see cref="BugsMqEnvelopeConsumer.Consume"/> to turn a recorded nack into a MassTransit fault, since MassTransit only settles a delivery as rejected when the consumer throws.</summary>
    ...
    /// <summary>Thrown by <see cref="BugsMqEnvelopeConsumer.Consume"/> when the handler itself fails unexpectedly, carrying the message identity MassTransit's own fault logging otherwise lacks.</summary>
