...
public static class ServiceCollectionExtensions
{
    ...
    /// pipeline <c>AddBugsMqRabbitMq</c> uses. <paramref name="configure"/> only ever touches
    ...
    /// deliberately not exposed here, so this call matches <c>AddBugsMqRabbitMq</c>'s shape exactly rather
    /// than leaking a second, adapter-specific configuration model into callers.
    /// </summary>
    public static IServiceCollection AddBugsMqMassTransit(this IServiceCollection services, Action<MassTransitOptions>? configure = null)
    {
        ...
        services.AddMassTransit(x =>
        {
            x.UsingRabbitMq((_, cfg) =>
            {
                ...
                // Every BugsMQ message shares this one MassTransit contract — see BugsMqEnvelopeMessage's
                ...
                cfg.Message<BugsMqEnvelopeMessage>(m => m.SetEntityName(options.ExchangeName));
                cfg.Publish<BugsMqEnvelopeMessage>(p => p.ExchangeType = "topic");
                cfg.Send<BugsMqEnvelopeMessage>(s => s.UseRoutingKeyFormatter(context => context.Message.MessageTypeName));
            });
        });
