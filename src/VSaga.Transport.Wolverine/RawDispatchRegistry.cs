...
public sealed class RawDispatchRegistry
{
    ...
    public async Task DispatchAsync(Envelope envelope, CancellationToken cancellationToken)
    {
        var address = envelope.Listener?.Address
            ?? throw new InvalidOperationException("VSaga Wolverine transport received an envelope with no listener address.");

        if (!_handlers.TryGetValue(address, out var handler))
            throw new InvalidOperationException($"No active VSaga subscription is registered for listener '{address}'.");

        var wire = JsonSerializer.Deserialize<WireEnvelope>(envelope.Data ?? [])
            ?? throw new InvalidOperationException("VSaga Wolverine transport received an envelope with an empty or malformed body.");
...
/// <summary>Thrown to fault Wolverine's own Handle invocation when the downstream VSaga handler nacked — see <see cref="WolverineAckContext"/>.</summary>
public sealed class WolverineRawMessageRejectedException(string messageTypeName, Guid correlationId)
    : Exception($"VSaga handler rejected '{messageTypeName}' for correlation id '{correlationId}'; routed to Wolverine's own error queue.")
{
