...
/// or any workflow/scheduler feature. BugsMQ.Core's SagaOrchestrator already owns retry, redelivery,
...
public sealed class BrighterTransport : IMessageTransport
{
    ...
    private static ReceivedMessage BuildReceivedMessage(Message message, RmqMessageConsumer consumer)
    {
        ...
        // RabbitMqTransport's raw AMQP headers never have. Filtering to the "x-bugsmq-" prefix keeps
        // ReceivedMessage.Headers limited to what BugsMQ itself ever writes (every real BugsMQ header,
        ...
        {
            if (value is null || !key.StartsWith("x-bugsmq-", StringComparison.Ordinal))
                continue;
    ...
    private sealed class BrighterAckContext(RmqMessageConsumer consumer, Message message) : IMessageAckContext
    {
        ...
        // BugsMQ.Core only ever calls this with requeue:false today (bounded redelivery is entirely
