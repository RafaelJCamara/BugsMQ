...
public static class TopologyRecordingServiceCollectionExtensions
{
    ...
    /// AddBugsMqRabbitMq/AddBugsMqInMemoryTransport (so there's a transport to wrap) and after
    /// AddBugsMqEfCore/AddBugsMqInMemoryPersistence if you want the recording to persist (otherwise it
    /// safely no-ops against <see cref="NullServiceTopologyStore"/>).
    /// </summary>
    public static IServiceCollection AddBugsMqTopologyRecording(this IServiceCollection services)
    {
        ...
        if (existing?.ImplementationFactory is null)
        {
            throw new InvalidOperationException(
                "AddBugsMqTopologyRecording requires an IMessageTransport factory registration already present " +
                "— call AddBugsMqRabbitMq or AddBugsMqInMemoryTransport first.");
        }
