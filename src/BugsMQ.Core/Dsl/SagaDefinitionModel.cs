...
{
    public readonly Dictionary<string, Dictionary<Type, StepDefinition<TState>>> StepsByState = new();

    public readonly Dictionary<string, Func<ISagaContext<TState>, CancellationToken, Task>> Compensations = new();

    public readonly Dictionary<string, (TimeSpan Delay, StepDefinition<TState> Step)> Timeouts = new();
    ...
    {
        foreach (var stateName in statesMostRecentFirst)
        {
            if (Compensations.TryGetValue(stateName, out var compensation))
