...
composition, the last item from that note, is *mostly* covered: a parent can start a child, the
relationship is persisted and queryable ("Sub-saga composition: parent linkage" below), and a child can
now report back to a parent that actually waits for it ("Sub-saga composition: completion notification"
below) — what's still missing is an engine-published safety net for a child that fails or times out
before it ever reaches its own report-back step. The SignalR hub and polling service, listed here as an
untested gap through several passes, are covered as of "SignalR hub and polling service tests".
...
- [`docs/sub-saga-composition.md`](docs/sub-saga-composition.md) — sub-saga composition's remaining two
  pieces: an engine-published safety net for a child that fails or times out before reporting back
  (Slice 2b), and whether a parent's compensation cascades into its children (Slice 3, the one the doc
  recommends pushing back on). Slices 1 and 2a are built and described below; the doc now carries what
  building them taught, including one claim it made that turned out to be wrong and one race condition
  it didn't anticipate at all.
