...
composition, the last item from that note, is *partly* covered: a parent can start a child and the
relationship is persisted and queryable ("Sub-saga composition: parent linkage" below), but a child
still cannot report back to its parent. The SignalR hub and polling service, listed here as an
...
- [`docs/sub-saga-composition.md`](docs/sub-saga-composition.md) — the remaining two slices of sub-saga
  composition: how a child reports completion, and whether a parent's compensation cascades into its
  children. Slice 1 of that design is built and described below; the doc now carries what building it
  taught, including one claim it made that turned out to be wrong.
